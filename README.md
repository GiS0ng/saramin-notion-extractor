...
- 마감일은 현재 공고의 접수정보 → 접수기간 → `validThrough`/`JobPosting` 구조화 데이터 → 상세요강 순서로 확인합니다. 이미지형 공고는 OCR 실행 후 누락 날짜를 보충합니다.
- 하나의 목록에 시작일·마감일이 함께 있는 구조, 표 형태, 점·하이픈·슬래시·한글 날짜, 두 자리 연도, 접수기간 범위, `D-N`/오늘·내일 마감을 처리합니다. 상대일자는 한국 시간을 기준으로 계산합니다.
- 연도 없는 월·일은 수집 시점의 한국 연도를 기준으로 하며 12월↔1월 경계를 보정합니다. 범위의 시작일에 연도가 있으면 그 연도를 우선 사용합니다. 과거 공고의 연도 없는 날짜는 미리보기에서 확인하세요.
- `상시채용`·`채용시 마감`처럼 확정 날짜가 없으면 Notion 날짜는 비워 둡니다. 미리보기 JSON의 `추출정보`에 `마감일상태`, `마감일원문`, `마감일출처`를 기록해 추출 실패와 구분합니다. 추가 Notion 속성은 필요하지 않습니다.
- 기술 스택은 제목·상세요강·요약 및 기술 태그·OCR에서 탐지합니다. 웹/앱, 백엔드, DB, 클라우드, 데이터/AI, 테스트, 제조 도구를 포함하며 영어·한글 별칭을 표준 이름으로 통합합니다. `C`, `C++`, `C#`를 구분합니다.
- 사전에 없는 기술이나 OCR이 잘못 읽은 글자는 누락될 수 있습니다. 새 기술은 `core.js`의 `SKILL_CATALOG`, 표기 변형은 `SKILL_ALIASES`에 추가하세요.
- 수정본 적용 시 Chrome 확장 프로그램을 새로고침하고 열려 있던 사람인 공고 탭도 새로고침한 뒤 다시 스캔하세요. 기존 Notion 항목은 재수집·저장하면 같은 공고 링크로 업데이트됩니다.
