...
1. Notion에서 내부 연결을 만들고 읽기·삽입·업데이트 권한을 부여합니다.
2. 아래 속성을 가진 데이터 소스를 연결에 공유합니다.
3. 확장 프로그램의 **Notion 연결 설정**에서 토큰과 데이터 소스 ID를 입력하고 연결 테스트를 실행합니다.
...
토큰과 데이터 소스 ID는 소스 코드나 fixture에 포함되지 않으며 Chrome 확장 프로그램 전용 로컬 저장소에만 저장됩니다. 공용 PC에서는 사용 후 **저장된 토큰 삭제**를 누르세요.
